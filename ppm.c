...
int ImageLoad_PPM(char *filename, Image *img) {
    ...
    /* allocation memoire */
    size = img->sizeX * img->sizeY * 3;
    img->data = (GLubyte *)realloc(img->data,(size_t)size * sizeof(GLubyte));
    assert(img->data);
...
void grune(Image *img) {
    ...
    for (crt = img->data, i = 0; i < nb_pixels; i++) {
        tmp =
            ((*crt * coef[0]) + (*(crt + 1) * coef[1]) + (*crt + coef[2])) >> 3;
        *crt++ = val2;
        // *crt++ = val1; /* L'image tout en vert */
        // crt++; /* filtre vert */
        *crt++ = tmp; /* filtre vert en prenant en compte la luminosité */
        /* Problème: la moyenne purement arithmétique n'est pas bonne car on est
         * pas autant sensible entre le vert, le rouge et le bleu. Il faut donc
         * rajouter des coefficients : 2xR, 4xG, 1xB */
        *crt++ = val2;
    }
}

void sobel(Image *img) { (void)*img; }

void gris(Image *img) { (void)*img; }
